...
class ThreeProxyManager:
    ...
    def generate_config(self, proxy_instances, backconnect_ip: str) -> str:
        """Генерирует конфигурацию 3proxy с правильными командами для HTTP и SOCKS5"""
        config_lines = [
            "daemon",
            "nserver 1.1.1.1",
        ...
        elif self.config.allowed_hosts:
            config_lines.extend([
                f"allow * * {self.config.allowed_hosts}",
                "deny *"
            ])
        else:
            config_lines.append("allow *")
        ...
        for instance in proxy_instances:
            ...
            proxy_line = self._get_proxy_line(instance, backconnect_ip)
            config_lines.append(proxy_line)

        return "\n".join(config_lines)

    def _get_proxy_line(self, instance, backconnect_ip: str) -> str:
        """
        ИСПРАВЛЕНИЕ: Генерирует строку конфигурации для одного прокси с правильными командами
        - HTTP прокси: используем команду 'proxy'
        - SOCKS5 прокси: используем команду 'socks'
        """
        if self.config.proxy_type.value == "http":
            # Для HTTP прокси используем команду 'proxy'
            proxy_cmd = "proxy -n -a"
        else:  # SOCKS5
            # Для SOCKS5 используем команду 'socks'
            proxy_cmd = "socks -a"

        return f"{proxy_cmd} -p{instance.port} -i{backconnect_ip} -e{instance.ipv6_address}"
